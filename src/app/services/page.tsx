  ...
  return (
    <section className="relative min-h-screen py-20 px-4 sm:px-6 lg:px-16 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto space-y-20">
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-bold mb-8 text-center text-blue-shades-300 drop-shadow-lg">What I Do</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {technicalServices.map(({ icon, title, items }, idx) => (
              <motion.div
                key={title}
                className="group bg-slate-900/80 backdrop-blur-md rounded-2xl p-8 shadow-2xl border border-blue-shades-900/30 hover:border-blue-shades-400 hover:shadow-blue-shades-900/20 transition-all duration-300 flex flex-col items-start"
              ...
              >
                <div className="text-blue-shades-400 mb-4 group-hover:text-purple-shades-300 text-4xl transition-colors duration-300 animate-pulse">
                  {icon}
                </div>
                <h3 className="text-2xl font-semibold mb-2 group-hover:text-white transition-colors duration-300">
                  {title}
                </h3>
                <ul className="list-disc pl-5 text-blue-shades-200/90 text-sm space-y-1 mt-2">
                  {items.map((item, i) => (
        ...
        <motion.div
          ...
          className="mt-20"
        >
          <h2 className="text-2xl sm:text-3xl font-bold mb-8 text-center text-blue-shades-200 drop-shadow">Soft Skills</h2>
          <div className="flex flex-wrap justify-center gap-6">
            {softSkills.map(({ icon, label }, idx) => (
              <motion.div
                key={label}
                className="flex items-center gap-3 bg-slate-900/80 backdrop-blur-md rounded-full px-6 py-3 shadow-lg border border-blue-shades-900/30 hover:border-purple-shades-400 hover:shadow-purple-shades-900/20 transition-all duration-300 text-blue-shades-100 text-lg font-medium"
              ...
              >
                <span className="text-purple-shades-400 text-2xl">{icon}</span>
                {label}
              </motion.div>
            ))}
          </div>
